...
export default function ProjectsSettingsPage() {
  ...
  const resetForm = () => {
    setEditingId(null);
    setName(''); setDescription(''); setTags([]); setCurrentTag(''); setProjectUrl('');
    setGalleryPreviews([]); setGalleryFiles([]);
    setVideoPreview(null); setVideoFile(null);
    setVideoThumbPreview(null); setVideoThumbFile(null);
  ...
  return (
    <div className="flex flex-col gap-8">
      {/* --- FORM SECTION --- */}
      <section>
        <h2 className="text-lg font-medium text-zinc-800 mb-3">{editingId ? "Edit Project" : "Add New Project"}</h2>
        <form onSubmit={handleSaveProject} className="flex flex-col gap-4 p-4 rounded-lg border border-zinc-200">
          ...
          <div>
            ...
            <input type="text" value={name} onChange={e => setName(e.target.value)} className="mt-1 w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm" placeholder="My Awesome Project" />
          </div>
          <div>
            <label className="text-sm font-medium text-zinc-700">Description</label>
            <textarea value={description} onChange={e => setDescription(e.target.value)} rows={3} className="mt-1 w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm" placeholder="Tell us about it..." />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* INPUT GALERI */}
            <div>
              <label className="text-sm font-medium text-zinc-700 mb-1 block">Photos (Max 7)</label>
              <div className="flex flex-wrap gap-2 mt-2">
                {galleryPreviews.map((src, idx) => (
                  <div key={idx} className="relative w-16 h-16 rounded-md overflow-hidden border border-zinc-200 group">
                    <img src={resolveIpfsUrl(src)} alt={`Gal ${idx}`} className="w-full h-full object-cover" />
                    <button type="button" onClick={() => removeGalleryItem(idx)} className="absolute top-0 right-0 bg-black/50 text-white p-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                      <XIcon />
                    </button>
                  </div>
                ))}
                {galleryPreviews.length < 7 && (
                  <button type="button" onClick={() => galleryInputRef.current?.click()} className="w-16 h-16 flex items-center justify-center rounded-md border-2 border-dashed border-zinc-300 hover:bg-zinc-50 text-zinc-400">
                    <PlusIcon />
                  </button>
                )}
              </div>
              <input type="file" ref={galleryInputRef} onChange={handleGalleryChange} multiple accept="image/*" className="hidden" />
            </div>

            {/* INPUT VIDEO */}
            <div>
              <label className="text-sm font-medium text-zinc-700 mb-1 block">Video (Max 3 mins)</label>
              {!videoPreview ? (
                <button type="button" onClick={() => videoInputRef.current?.click()} className="mt-2 w-full h-32 flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-zinc-300 hover:bg-zinc-50 text-zinc-500 text-sm">
                ...
                <div className="mt-2 flex flex-col gap-2">
                  <div className="relative w-full aspect-video bg-black rounded-lg overflow-hidden group">
                    <video src={resolveIpfsUrl(videoPreview)} className="w-full h-full object-contain" controls />
                    <button type="button" onClick={removeVideo} className="absolute top-2 right-2 bg-red-600 text-white text-xs px-2 py-1 rounded shadow opacity-0 group-hover:opacity-100 transition-opacity">
                      Remove
                    </button>
                  </div>
                  ...
                  <div className="flex items-center gap-3 mt-1 p-2 bg-zinc-50 rounded-md border border-zinc-200">
                    <div className="w-10 h-10 bg-zinc-200 rounded overflow-hidden flex-shrink-0">
                      {videoThumbPreview ? (
                        <img src={resolveIpfsUrl(videoThumbPreview)} className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-[8px] text-zinc-400">Cover</div>
                      )}
                    </div>
                    <div className="flex-1">
                      <p className="text-xs font-medium text-zinc-700">Video Thumbnail</p>
                      <button type="button" onClick={() => videoThumbInputRef.current?.click()} className="text-[10px] text-blue-600 hover:underline">
                        {videoThumbPreview ? "Change Cover" : "Upload Cover Image"}
                      </button>
                    </div>
                    <input type="file" ref={videoThumbInputRef} onChange={handleVideoThumbChange} accept="image/*" className="hidden" />
                  </div>
                </div>
              )}
          ...
          <div>
            <label className="text-sm font-medium text-zinc-700">Project Link</label>
            <input type="url" value={projectUrl} onChange={e => setProjectUrl(e.target.value)} placeholder="https://..." className="mt-1 w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm" />
          </div>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-1 w-full rounded-lg border border-zinc-300 px-3 py-2">
              {tags.map(tag => (
                <span key={tag} className="flex items-center gap-1 rounded-full bg-sky-100 px-2.5 py-0.5 text-xs font-medium text-sky-800">
                  {tag} <button type="button" onClick={() => removeTag(tag)}><XIcon /></button>
                </span>
              ))}
              <input type="text" value={currentTag} onChange={e => setCurrentTag(e.target.value)} onKeyDown={handleTagInput} placeholder="Add tag..." className="flex-1 text-sm focus:outline-none" />
            </div>
          </div>

          <div className="flex justify-end gap-2 mt-2">
            {editingId && <button type="button" onClick={resetForm} className="rounded-lg bg-zinc-100 px-4 py-2 text-sm font-medium text-zinc-800">Cancel</button>}
            <button type="submit" className="rounded-lg bg-zinc-900 px-4 py-2 text-sm font-medium text-white">Save Project</button>
          </div>
        </form>
      </section>
...
const ProjectListItem = ({ 
  ...
  return (
    <div className="flex items-center gap-3 w-full rounded-lg border border-zinc-200 p-3 bg-white hover:border-zinc-300 transition-colors">
      ...
      <div className="flex-shrink-0 flex items-center gap-2 md:gap-3">
        ...
        <div className="hidden md:flex gap-1">
            ...
            <span key={tag} className="rounded-full bg-sky-100 px-2 py-0.5 text-[10px] font-medium text-sky-800">
                {tag}
            </span>
            ))}
        </div>
        ...
        <button
          ...
          className={`p-1.5 rounded hover:bg-zinc-100 text-zinc-400 hover:text-yellow-500 ${project.isFeatured ? 'text-yellow-500' : ''}`}
          title={project.isFeatured ? "Remove Featured" : "Mark as Featured"}
        >
          <StarIcon filled={project.isFeatured} />
        </button>
        <button onClick={() => onEdit(project)} className="p-1.5 rounded hover:bg-zinc-100 text-zinc-500 hover:text-zinc-900" title="Edit">
          <EditIcon />
        </button>
        <button onClick={() => onDelete(project.id)} className="p-1.5 rounded hover:bg-red-50 text-zinc-500 hover:text-red-600" title="Delete">
          <TrashIcon />
        </button>
      </div>
    </div>
