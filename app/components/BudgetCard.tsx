...
export default function BudgetCard() {
  ...
  const estimatedEth = ethPrice && usdAmount 
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-lg font-semibold text-zinc-900">Protocol Budget (USD)</h2>
          <p className="text-xs text-zinc-500">Deposit budget in USD value (paid in ETH) for instant AI access.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {/* Kolom Kiri: Info Saldo (Tetap dalam ETH karena di DB kita simpan ETH) */}
        <div className="flex flex-col justify-center p-4 bg-zinc-50 rounded-lg border border-zinc-100">
          <span className="text-xs font-medium text-zinc-500 uppercase tracking-wider mb-1">Current Balance</span>
        ...
        <div className="flex flex-col gap-3">
           <label className="text-sm font-medium text-zinc-700">Top-up Amount (USD)</label>
           ...
           <div className="relative">
             ...
             <input 
               ...
               placeholder="5"
               className="w-full rounded-lg border border-zinc-300 pl-7 pr-4 py-2 text-sm focus:ring-2 focus:ring-zinc-900 focus:border-transparent outline-none transition-all font-medium"
             />
           </div>
           ...
           <button
             ...
             className="w-full mt-1 rounded-lg bg-zinc-900 px-4 py-2.5 text-sm font-medium text-white transition-all hover:bg-zinc-800 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
           >
             {isDepositing ? (
               <>
